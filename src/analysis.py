...
def load_graph_from_edges(
    ...
    """Load weighted directed graph from CSV edge list."""
...
def compute_betweenness(graph: nx.DiGraph) -> dict:
    """Correct betweenness centrality using edge weights."""
...
def node_metrics_ranking(graph: nx.DiGraph, top_n: int = 20) -> pd.DataFrame:
    """Compute node-level communication metrics."""
...
def save_summary(summary: dict, output_path: str | Path) -> None:
